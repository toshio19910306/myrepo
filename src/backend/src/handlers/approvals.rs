...
use crate::{
    ...
    services::{approval_service, email_service::EmailService},
...
async fn process_approval_action(
    ...
    let email_service = EmailService::new(state.config.sendgrid_api_key.clone());
    
    match approval_service::process_approval_action(&state.db_pool, &email_service, id, payload).await {
        Ok(flow) => Ok(Json(ApiResponse {
            success: true,
            data: Some(json!({
                "flow_id": flow.flow_id,
