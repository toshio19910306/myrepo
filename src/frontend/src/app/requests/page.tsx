...
export default function RequestsPage() {
  ...
  const handleEditRequest = async (request: EstimateRequest) => {
    try {
      ...
      if (response.ok) {
        ...
        const filesResponse = await fetch(`/api/files/target/REQUEST/${request.request_id}`);
  ...
  const handleSubmitApproval = async () => {
    ...
    try {
      const response = await fetch(`/api/requests/${approvalRequestId}/submit-for-approval`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
